...
export class FakeBackendInterceptor implements HttpInterceptor {
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function register() {
            ...
            if (users.find(x => x.username === user.username)) {  //search for user in localstorage on usernmae
        ...
        function authenticate() { //validate the user
            const { username, password } = body;//get the credential from the body data of the request  object
        ...
        function getUsers() {//retreive users with logged in status 
            ...
            return ok(users);//else return logged in user
            //ok(), defined below simulates an all ok http response object with status code 200, from REST API
        }
        ...
        function ok(body?) {//simulates an all ok http response object with status code 200, from REST API
                            //? = indicates accpet data in the body paramer or accepts null also
            return of(new HttpResponse({ status: 200, body }))
        }
        function error(message) {//throws a custom error message passed to this function
