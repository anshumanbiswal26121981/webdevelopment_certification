...
export class AuthenticationService {
    ...
    login(username, password) { //custom method that accepts user credentials, logs in  the user, store in localstorage
        ...
        return this.http.post<any>(`${environment.apiUrl}/users/authenticate`, { username, password })
            .pipe(map(user => {
                ...
                this.currentUserSubject.next(user); //sends notification on the obervable 
                                                    //indicates authentication is done , can call the next method for user
    ...
    logout() {//custom method to remove user record from local storage and set current user to null
        ...
        this.currentUserSubject.next(null); //sends notification on the obervable ,indicates authentication is done 
